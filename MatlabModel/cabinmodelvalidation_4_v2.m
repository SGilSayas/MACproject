...
t = 1;
timestep =  1; %s
lab_input='G8_22C_MACoff_TCcell1'; %'G8_22C_MACoff_TCcell3';
struct=load(lab_input);
table=struct2table(struct);
lab_table=table2array(table);
array=table2array(lab_table(:,1)); % C
n=8.5;
TC_cell=array(1 : n : end);
TC_cell=TC_cell(:,:); 

lab_input='G8_22C_MACoff_TCcabin1';%'G8_22C_MACoff_TCcabin3';
struct=load(lab_input);
table=struct2table(struct);
lab_table=table2array(table);
TC_vent=table2array(lab_table(:,1)); % C
TC_front=table2array(lab_table(:,2)); % C
TC_driver=table2array(lab_table(:,3)); % C
TC_pass=table2array(lab_table(:,4)); % C
TC_back=table2array(lab_table(:,5)); % C
TC_backpass=table2array(lab_table(:,6)); % C

% Time
t = 1; timestep =  1; % [s]
Total_time = length(TC_front)-1; % 1800 WLTC duration %size(TC_front,1)-1;
...
% Ambient temperature: %T_ext = 273.15 + interp1(Temp_Ext(:,1),...
% Temp_Ext(:,2),time); Tamb=T_ext; T_ini=T_ext(1);
...
Tbc=[T_amb(t);-A_roof*G_roof_a(t);0;-Qbase(t);-A_ws*G_ws_a(t);...
    -A_rw*G_rw_a(t);-A_lsw*G_lsw_a(t);-A_rsw*G_rsw_a(t);0;0;0;0;...
    -Qhuman(t);-Qequipment(t)]; 
...
C_cabin_front=(density_air*(CabinVolume/2)*cp_air/timestep) + ...
    ((mass_seats/2)*cp_seats/timestep);
...
Tcabin_back(t)=T_ini;
temperature=[T_ini;T_ini;T_ini;T_ini;T_ini;T_ini;T_ini;T_ini;...
    T_ini;T_ini;T_ini;T_ini;T_ini;T_ini];
delta_temperature=[T_ini;T_ini;T_ini;T_ini;T_ini;T_ini;T_ini;...
    T_ini;T_ini;T_ini;T_ini;T_ini;T_ini;T_ini];
...
%     %if (temperature(13)>T_amb(t)+0.5 | temperature(14)>T_amb(t)+0.5 && temperature(14)<temperature(13)) % OR: | AND: &&
...
%     % Exchange from the front and back
    ...
    if (temperature(14)>T_amb(t) + i)%&& temperature(14)>temperature(13))
        ...
        U_back= 1 * 1/R_back; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ( x0.001 ) %%%%%%%
        temperature(14) = (U_back*A_back*temperature(1) + ...
            density_air*V_cell*delta_temperature(14)/timestep) / ...
            (U_back*A_back + density_air*V_cell/(timestep));         
        %temperature(14) = (h_cabin*cabin_height*cabin_width*temperature(13) + ...
        % density_air*CabinVolume*delta_temperature(14)/timestep) / ...
        % (h_cabin*cabin_height*cabin_width + density_air*CabinVolume/(timestep));
...
plot(time,Tamb-273.15,':g','LineWidth',1) %T amb simulated
plot(time,TC_front,'-.r','LineWidth',1.5)
plot(time,TC_back,':r','LineWidth',1.5)
hold off
legend('T front sim','T back sim','Tamb sim','TC front','TC back','Location','best')
grid on 
grid minor
xlabel('Time [s]') 
