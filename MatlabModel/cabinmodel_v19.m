...
if(WT==1)
    ...
    zone6 = readtable('windtunneldata.xlsx','Sheet','Sheet6');
    zone7 = readtable('windtunneldata.xlsx','Sheet','Sheet7');
    wt_data = [zone3;zone4;zone5;zone6;zone7];
    wt_data=table2array(wt_data);

    % Wind zones:
    x3 = 2*height(zone3);
    x4 = x3 + 2*height(zone4);
    x5 = x4 + 2*height(zone5);
    x6 = x5 + 2*height(zone6);
    x7 = x6 + 2*height(zone7);

    wt_data_rs=ones(length(wt_data)*2,8);
    j=1;
    while j<length(wt_data)
        for k=1:length(wt_data)*2-1 %1800
            if rem(k,2) ~= 0 % for odd (impar) k
                wt_data_rs(k,:) = wt_data(j,:);
            else % for even k
                wt_data_rs(k,:) = wt_data(j,:)+((wt_data(j+1,:)-wt_data(j,:))./2);
                j=j+1;
            end
            ...
            elseif k>x3 && k<=x4 % zone 4: 25 m/s = 90 km/h = high
                vehicle_kmh(k)=90;
            elseif k>x4 && k<=x5 % zone 5: 30 m/s = 108 km/h = extra high
                vehicle_kmh(k)=108;
            elseif k>x5 && k<=x6 % zone 6: 20 m/s = 72 km/h = medium
                vehicle_kmh(k)=72;
            elseif k>x6 % zone 7: 10 m/s =  km/h = low
                vehicle_kmh(k)=36;
            end
        end
    end
    TimeWT = wt_data_rs(1:end-6,1);
    TC_cell = wt_data_rs(1:end-6,2);
    TC_vent = wt_data_rs(1:end-6,3);
    TC_front = wt_data_rs(1:end-6,4)+0.2;
    TC_driver = wt_data_rs(1:end-6,5);
    TC_pass = wt_data_rs(1:end-6,6);
    TC_back = wt_data_rs(1:end-6,7)+0.2;
    TC_backpass = wt_data_rs(1:end-6,8);
...
% Inizialization of Heat transfer Convection coefficients, W/(m2*K)
h_front=5; 
h_rear=5;
h_base=5;
h_side=5;
...
K1=h_side*A_roof; %h_roof?
K2=A_roof*k_ceiling/e_ceiling;
K3=h_interior*A_roof;
...
if (WT==1)
    n_X = length(X)/(Total_time);
    X = interp1(X,1:n_X:(numel(X)+1));
    X = transpose(X);
end
...
e_base = 0.1; %m
U_engine = 1/( 1/h_front + 1/h_interior + e_enginesheet/k_steel );
U_exhaust = 1/( 1/h_front + 1/h_interior + e_base/k_steel );
% U_engine = h_ext;
% U_exhaust = h_cabin;

for t=2:Total_time
    f=1; %factor de escala maqueta
    ...
    if vehicle_kmh(t)<=56.5 % LOW
        ...
        U_front(t) = 60;
        U_back(t) = 30;
        %U_front(t) = 1 / (1/h_front(t)+(e_ws/k_ws)); 
    elseif vehicle_kmh(t)>56.5 && vehicle_kmh(t)<=76.6 % MEDIUM, 20 m/s
        h_front(t) = 1000;
        U_front(t) = 100;
        U_back(t) = 50; % it doesnt matter the num,
    elseif vehicle_kmh(t)>76.6 && vehicle_kmh(t)<=97.4 % HIGH, 25 m/s
        h_front(t) = 2000;
        U_front(t) = 300;
        U_back(t) = 165;
    elseif vehicle_kmh(t)>97.4 && vehicle_kmh(t)<=131.3 % EXTRA HIGH, 30 m/s
        h_front(t) = 80000;
        U_front(t) = 360;
        U_back(t) = 250;
    ...
    U_side(t) = 0.8*U_front(t);

    A_front = A_ws + A_dashboard; %3
    A_back = A_rw; %1.5
    A_side = A_sidewindows + A_doors;
    ...
    UA_front = A_front*U_front(t) + 1*(A_side*U_side(t));
    UA_back = A_back*U_back(t) + 1*(A_side*U_side(t));
    ...
    T_engine(t) = 0; % 273.15 + (-2*10^(-6)*engine_rpm(t)^(2) + 0.0355*engine_rpm(t) + 77.5); %K
    T_exhaust(t) = 0; % 273.15 + 0.138*engine_rpm(t) - 17; 
    ...
    heat_human(t) = h_interior*A_skin*abs(temperature(13)-T_skin) ;
    Qhuman(t) = N_Humans.*heat_human(t);
    Qequipment(t) = heat_equipment;
    Qbase(t) = 0;
    Qengine_calc(t) = 0; % A_engine*U_engine*abs(T_engine(t)-temperature(13));% - 11 - 44;
    Qexhaust_calc(t) = 0; %A_exhaust*U_exhaust*abs(T_exhaust(t)-temperature(14));%  + 17.77;
...
tx1 = ['Vehicle reached higher speed than WLTC limits during: ',num2str(high_speed_index),' seconds.'];
disp(tx1)
tx2 = ['Area front: ',num2str(A_front)];
disp(tx2)
tx3 = ['Area back: ',num2str(A_back)];
disp(tx3)
tx3 = ['Area side: ',num2str(A_side)];
disp(tx3)
...
tx4 = ['Total cabin load: ',num2str(total_cabin_load_W),' W'];
disp(tx4)
...
tx5 = ['Total MAC cooling load: ',num2str(total_mac_cooling_load_W),' W'];
disp(tx5)
    % AC compressor power demand
compressor_electric_power_W = -total_mac_cooling_load_W/COP;
compressor_cumsum = cumsum(Qmac_needed);
% tx6 = ['Compressor electric power: ',num2str(compressor_electric_power_W),' W'];
% disp(tx6)
...
hold on
plot(time(3:Total_time),Qmac_needed_plot(3:Total_time),'-k','LineWidth',1.5)
grid minor
ylabel('MAC Heat [W]')
xlabel('Time [s]')
if(G7_22==1)
...
%%
do_you_want_means=0; %or 1
%timeWTplots=1:1:length(time)-60; AND Tcabin_front(61:end)

%% figure(1) %% ONLY HEATING
% if WT==1
%     plot(time,Tcabin_front-273.15,':b','LineWidth',1.5)
%     hold on
%     plot(time,Tcabin_back-273.15,':m','LineWidth',1.5)
%     plot(time,TC_cell-273.15,'-k','LineWidth',1.5) %T amb 
%     plot(time,TC_front,'-b','LineWidth',1.5) 
%     plot(time,TC_back,'-m','LineWidth',1.5)
%     xline(x3)
%     xline(x4)
%     t1 = text(x3-200,26.5,'20 m/s','FontSize',12,'Color','k');
%     t2 = text(x4-300,26.5,'25 m/s','FontSize',12,'Color','k');
%     t3 = text(x4+250,26.5,'30 m/s','FontSize',12,'Color','k');
%     if do_you_want_means==1
%         plot(time,mean_TC_front,'--k','LineWidth',1) 
%         plot(time,mean_TC_back,'--k','LineWidth',1)
%         legend('T front simulated','T back simulated','TC ambient','TC front','TC back','TC front mean','TC back mean','Location','best')
%     end
...
if WT==1
    ...
    xline(x5)
    xline(x6)
    t1 = text(x3-250,27.5,'20 m/s','FontSize',10,'Color','k');
    t2 = text(x4-320,27.5,'25 m/s','FontSize',10,'Color','k');
    t3 = text(x4+250,27.5,'30 m/s','FontSize',10,'Color','k');
    t4 = text(x5+50,27.5,'20 m/s','FontSize',10,'Color','k');
    t5 = text(x6+50,27.5,'10 m/s','FontSize',10,'Color','k');
    if do_you_want_means==1
        plot(time,mean_TC_front,'--k','LineWidth',1) 
        plot(time,mean_TC_back,'--k','LineWidth',1)
        legend('T front simulated','T back simulated','TC ambient','TC front','TC back','TC front mean','TC back mean','Location','best')
...
figure(3)
plot(U_front,'-','LineWidth',1.5)
hold on
plot(U_side,'--','LineWidth',1.5)
plot(U_back,':','LineWidth',1.5)
legend('U front','U side','U back','Location','best')
xlabel('Time [s]') 
ylabel('Overall Heat Transfer Coefficients [W/(m^2 ºC)]')
xlim([1 2350])
% for heating: xlim([1 1400])
grid on
