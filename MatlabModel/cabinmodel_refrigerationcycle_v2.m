...
delta_T_evap = 5; % Automotive: 7-12  ; HVAC: 5-10
...
mf(t) = 0.1; % Compressor refrigerant mass flow [kg/s]
...
%%
    ...
    s2is(t)=s1(t); 
    %% new
    h2is(t)=py.CoolProp.CoolProp.PropsSI('H','T',T_cond(t),'S',s2is(t),Ref); % falls inside of saturated mix area, so we can use T_cond
...
%%
    ...
    % % FIXED value:     mf(t)=50/1000; %kg/s, small: 20-50 g/s, SUVS: 100-200, HDV: 200-500
    ...
    Q_MAC_zone = Q_MAC(t) / 2;
    temperature(13) = (Qcabin_req(t) / 2 - Q_MAC_zone) * (timestep / (V_cabin / 2 * cp_air * density_air)) + prev_temp(13);
    temperature(14) = (Qcabin_req(t) / 2 - Q_MAC_zone) * (timestep / (V_cabin / 2 * cp_air * density_air)) + prev_temp(14);
...
figure(10)
Tcabin_front=Tcabin_front(3:end);
Tcabin_back=Tcabin_back(3:end);
T_amb=T_amb(2:end);
TC_front=TC_front(2:end);
TC_back=TC_back(end);
