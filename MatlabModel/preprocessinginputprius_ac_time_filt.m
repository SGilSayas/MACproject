...
%% NEW
% Analyze `TC_cabin` to identify the most frequent ranges
bin_width = 1; % Define bin width for histogram (1 degree)
edges = min(TC_cabin):bin_width:max(TC_cabin); % Define histogram edges
[counts, edges] = histcounts(TC_cabin, edges); % Generate histogram

% Find the most common bins
[~, sorted_indices] = sort(counts, 'descend'); % Sort bins by frequency
top_bins = sorted_indices(1:2); % Take the top 2 bins (adjust as needed)

% Define the temperature ranges based on the top bins
temperature_ranges = [edges(top_bins)', edges(top_bins + 1)'];

% Initialize output tables
valid_output_table = table();
invalid_trip_ids = [];
%%
...
for i = 1:length(unique_trip_ids)
    ...
    % Skip trips with less than 1 A in AC compressor
    if trip_ac_filtered <= 0.650 % kW
        invalid_trip_ids = [invalid_trip_ids; current_trip_id];
        continue;
    end
end
...
for g = 1:size(group_data, 1)
    if ~isempty(group_data{g, 3})
        ...
        % Skip trips with less than 1 second duration
        if trip_durations_inseconds <= 1
            invalid_trip_ids = [invalid_trip_ids; current_trip_id];
            continue;
        end
        
        % Check if ac_consumption_filtered is 0 for more than 95% of the trip
        zero_ac_time_fraction = sum(trip_ac_filtered == 0) / length(trip_ac_filtered);
        if zero_ac_time_fraction > 0.95
            invalid_trip_ids = [invalid_trip_ids; current_trip_id];
            continue;
        end
        
        % Check if cabin temperature is within any identified range for at least 50% of the trip
        meets_temperature_condition = false;
        for range = temperature_ranges'
            lower_bound = range(1);
            upper_bound = range(2);
            in_range_fraction = sum(trip_TC_cabin >= lower_bound & trip_TC_cabin < upper_bound) / length(trip_TC_cabin);
            if in_range_fraction >= 0.5
                meets_temperature_condition = true;
                break;
            end
        end
        
        if ~meets_temperature_condition
            invalid_trip_ids = [invalid_trip_ids; current_trip_id];
            continue;
        end
        ...
        unique_trips = unique(filtered_output_table.trip_id);
        active_time_per_trip = zeros(length(unique_trips), 1);
        for t = 1:length(unique_trips)
            trip_mask = filtered_output_table.trip_id == unique_trips(t);
            trip_data = filtered_output_table(trip_mask, :);
        ...
        filtered_output_table.active_time = zeros(height(filtered_output_table), 1);
        for t = 1:length(unique_trips)
            trip_mask = filtered_output_table.trip_id == unique_trips(t);
            filtered_output_table.active_time(trip_mask) = active_time_per_trip(t);
        end
        
        valid_output_table = [valid_output_table; filtered_output_table];

        % Save the output table for the current group in the struct array
        group_tables(g).group_id = g;
        group_tables(g).output_table = valid_output_table;

        % % % Write the output table to the cell array
        % % output_table = filtered_output_table;
        % % group_output_tables{g} = valid_output_table;

        % Write filtered data with new 'trip_time' column to Excel
        % writetable(valid_output_table, group_data{g, 1});       
        
    end
end
% Save the valid trips to an Excel file
writetable(valid_output_table, 'valid_trip_data.xlsx');
disp(invalid_trip_ids(i));
