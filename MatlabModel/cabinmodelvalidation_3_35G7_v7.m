...
%% OP1:
% alpha=0.1; % alpha = (efficiency from mech power to thermal)/(mass flow warm air from engine compartment to cabin, m_aireng kg/s)
% T_engine(t) = T_amb(t) + engine_kW*alpha/cp_air;
%% OP2:
%T_engine(t)=-2*10^(-6)*engine_rpm(t)^(2) + 0.0355*engine_rpm(t) + 77.5; %Fayazbakhsh 2013
%%



for t=2:Total_time
    V_cell=V_cell; % *1.0003;
    ...
    Qengine_calc(t)= A_engine*U_engine*(T_engine(t)-temperature(13)) - 11;
    Qexhaust_calc(t)= A_exhaust*U_exhaust*(T_exhaust(t)-temperature(14)) + 17.77;

    %% OP 2: works very well, but does it make sense?
    ...
    %(temperature(13)-delta_temperature(13))*density_air*V_cell/timestep = (T_amb(t)*0.998-temperature(13))*UA_front;
    ...
    if ( temperature(13)>T_amb(t) ) % si T_amb(t)*0.998 all ok
        ...
        %temperature(13)=(inv(K(13,13) - C(13,13)))*(Tbc(13) - C(13,13)*delta_temperature(13));
    ...
    Qmac_needed(t)= (CabinVolume*density_air*cp_air + mass_seats*cp_seats+144240)*... % + 144240 ?
        (((temperature(13)+temperature(14))/2)-Ttarget)/tcons;
    ...
    Qcabin_received(t)=(Qhuman(t) + Qequipment(t) + Qengine(t) + Qexhaust(t) + Qcv_received(t));
