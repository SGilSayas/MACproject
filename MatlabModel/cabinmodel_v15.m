% Vehicle's Cabin Model, Susana Gil-Sayas. Version: 15
clc
clear all 
close all
...
%        Qcv_emitted(t) = Qcv_emitted(t)-(temperature(14)-TC_cell(t))*UA_back;
    ...
    if(heatpump==0)
        if(Tcabin(t)>Ttarget)
            ...
            Qtarget(t) = (CabinVolume*density_air*cp_air + mass_seats*cp_seats)*(Ttarget-Tcabin(t))/(tcons(t))
    ...
    elseif(heatpump==1)
        Qtarget(t) = (CabinVolume*density_air*cp_air + mass_seats*cp_seats+144240)*(Ttarget-Tcabin(t))/ttarget(t); % negative: cooling, positive: heating
        Qmac_needed(t) = Qcabin_received(t) + Qcv_emitted(t) + Qtarget(t);
    end
...
evaporator_av_W = sum(Qmac_needed)/Total_time;
compressor_av_W = -evaporator_av_W/COP
...
elseif(G8_22==1)
    ylim([-400 200])
elseif(G7_35==1 || G8_35==1)
    ylim([-5000 -3000]) % G7_35: ylim([-21000 -18500]) % G8_35: ylim([-20000 -13000])
end
yyaxis right
ax = gca;
ax.YColor = [1 0 0];
