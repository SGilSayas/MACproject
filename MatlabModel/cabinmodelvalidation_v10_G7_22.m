...
% for k=2:Total_time 
%     if ( T_amb(k)>mean(T_amb)+0.8 || T_amb(k)<mean(T_amb)-0.8 )
%         T_amb(k)=T_amb(k-1);        
%     end
% end
% p=5; % p=mean(T_amb);
% for k=2:Total_time 
%     if(T_amb(k)>p)
%         T_amb(k)=T_amb(k)*0.99; %(time(k).^2)/-(4*p); 
%     elseif(T_amb(k)<p)
%         T_amb(k)=T_amb(k)*1.01; %(time(k).^2)/(4*p); 
%     end
% end


if (change_signal==1)
    Local_Maximas = islocalmax(T_amb);
    Indicies = find(Local_Maximas == 1);
    Period = time(1,Indicies(1,2)) - time(1,Indicies(1,1));
    B=0.745*(2*pi/Period);
    C=100;
    T_ini_100=T_amb(2);
    T_amb = 0.5.*cos(B.*time + B*C) + T_ini_100;
    % y = A sin(B(x + C)) + D
    % amplitud es A: del 0 a la y max, periodo es 2π/B: x entre picos de y, 
    % desfase es C (a la izquierda es positivo), desplazamiento vertical es D
end
...
TC_back= TC_back - abs(T_amb(2)-273.15-TC_back(2));% +1.5;
...
for t=2:Total_time
    ...
    Qleakage(t) = -0.0002*density_air*(e_amb(t)-e_cabin(t)); %vent_massflowrate % W, J/s
    Qvent(t) = vent_massflowrate*density_air*(e_amb(t)-e_cabin(t)); % W, J/s
    ...
    Qengine_observed_G7_22(t) = cp_air*CabinVolume/2*density_air*(delta_temp_engine_mean(t))/timestep;
    Qexhaust_observed_G7_22(t) = cp_air*CabinVolume/2*density_air*(delta_temp_exhaust_mean(t))/timestep;
...
% save Engine_thermal_loads_W_G7_22_v10 Qengine_observed_G7_22 Qexhaust_observed_G7_22
...
%%
figure(10)
plot(time,Qcabin_received,':k','LineWidth',1.5)
hold on
...
hold on
plot(time,Qmac_needed,'-','LineWidth',1.5)
...
% figure(5)
% colororder({'k','k'})
% plot(time,Qcabin_received,':b','LineWidth',1.5)
% grid minor
