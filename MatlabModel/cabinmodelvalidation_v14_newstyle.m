...
t = 1; 
COP = 3.5;
heatpump = 0; % 0 for no, 1 for yes

%% Load calculated engine and exhaust thermal load of test G7 at 35C

% % Not good engine heat profile %%
% engineheatinput = 'Engine_thermal_loads_W_G7_22_v10';
% load(engineheatinput);
% Qengine_observed = Qengine_observed_G7_22; 
% Qexhaust_observed = Qexhaust_observed_G7_22;
...
% % % Not good engine heat profile %%
% engineheatinput = 'Engine_thermal_loads_W_G8_22_v10';
% load(engineheatinput);
% Qengine_observed = Qengine_observed_G8_22; 
% Qexhaust_observed = Qexhaust_observed_G8_22;

% engineheatinput = 'Engine_thermal_loads_W_G8_35_v10';
% load(engineheatinput);
% Qengine_observed = Qengine_observed_G8_35;
% Qexhaust_observed = Qexhaust_observed_G8_35;
...
deletion_peak = 0; % 1 for yes, 0 for no
...
if(G8_35==1)
    ...
    TC_front=(TC_pass.*0.4+TC_backpass.*0.6);
    % TC_front=TC_driver;%(TC_pass.*0.5+TC_driver.*0.5);
end
...
% if(G7_22==1)
%     Local_Maximas = islocalmax(TC_cell);
%     Indicies = find(Local_Maximas == 1);
%     Period = time(1,Indicies(1,2)) - time(1,Indicies(1,1));
%     B=0.745*(2*pi/Period);
%     C=100;
%     T_ini_100=TC_cell(2);
%     TC_cell = 0.5.*cos(B.*time + B*C) + T_ini_100;
%     % y = A sin(B(x + C)) + D
%     % amplitud es A: del 0 a la y max, periodo es 2π/B: x entre picos de y, 
%     % desfase: C (a la izquierda es positivo), desplazamiento vertical: D
% end

% Noise deletion
TC_amb = movmean(TC_cell,10);
T_cell = movmean(TC_cell,10);
...
if(G7_22==1)
    TC_front= TC_front - (TC_front(6)-TC_amb(1)); %abs(
    TC_back= TC_back - (TC_back(6)-TC_amb(1)); %abs(
    TC_vent= TC_vent - (TC_vent(31)-TC_amb(1));
    TC_driver= TC_driver - (TC_driver(8)-TC_amb(1));
    % TC_pass= TC_pass - abs(TC_pass(2)-TC_amb(2));; % or middle
    TC_backpass= TC_backpass - (TC_backpass(2)-TC_amb(1)); % or equipment
elseif(G7_35==1)
    TC_front= TC_front + abs(TC_front(81)-TC_amb(81));
    TC_back= TC_back + abs(TC_back(2)-TC_amb(2));
    TC_vent= TC_vent + abs(TC_vent(2)-TC_amb(2));
    TC_driver= TC_driver + abs(TC_driver(2)-TC_amb(2));
    % TC_pass= TC_pass - abs(TC_pass(2)-TC_amb(2));; % or middle
    TC_backpass= TC_backpass - abs(TC_backpass(2)-TC_amb(2)); % or equipment
elseif(G8_22==1)
    % TC_front= TC_front -(25-21.8);
    % TC_back= TC_back -(25.8-21.6);
    TC_front= TC_front - abs(TC_front(70)-TC_amb(70))+0.5;
    TC_back= TC_back - abs(TC_back(70)-TC_amb(70))+0.5;
    TC_vent= TC_vent - abs(TC_vent(200)-TC_amb(200))+0.5;
    TC_driver= TC_driver - abs(TC_driver(200)-TC_amb(200))+0.5;
    % TC_pass= TC_pass - abs(TC_pass(2)-TC_amb(2));; % or middle
    TC_backpass= TC_backpass - abs(TC_backpass(200)-TC_amb(200))+0.5; % or equipment
elseif(G8_35==1)
    % TC_front= TC_front -0.5;
    % TC_back= TC_back -1;
    TC_front= TC_front - (TC_front(90)-TC_amb(90)) ;
    TC_back= TC_back - (TC_back(90)-TC_amb(90)) ;
    TC_vent= TC_vent - (TC_vent(34)-TC_amb(34));
    TC_driver= TC_driver - (TC_driver(2)-TC_amb(2));
    % TC_pass= TC_pass - abs(TC_pass(2)-TC_amb(2));; % or middle
    TC_backpass= TC_backpass - (TC_backpass(2)-TC_amb(2)); % or equipment
end


% Transpose TC amb vector and initialization
TC_amb=transpose(TC_amb)+273.15; % T_amb in K. Both TC_cell or T_cell_OBD
T_ini=TC_amb(1);

% Reductor of amplitud
if (reduced_amplitude==1)
    signal = TC_amb;
    temp_signal = signal / reduction;
    T_amb = (mean(signal)) - (mean(temp_signal))  + temp_signal;
else
    T_amb=TC_amb;
end

% Deletion high peak in G8 22C TC cell data:
if(G8_22==1 && deletion_peak ==1)
    for k=1:Total_time
        if(k > 1600)
            T_amb(k)=TC_amb(k-200)+0.4;
        else
            T_amb(k)=TC_amb(k);
        end
    end
...
for t=2:Total_time
   ...
   % Exchange from the front and back:
    if ( temperature(13)>T_amb(t) )
        temperature(13)= ( density_air*CabinVolume/4*cp_air*delta_temperature(13)/timestep + UA_front*T_amb(t) )/...
    ...
    if ( temperature(14)>T_amb(t) )
        ...
        temperature(14) = (UA_back*T_amb(t) + density_air*CabinVolume/4*cp_air*delta_temperature(14)/timestep)...
...
% hold on
% plot(time,Qcv_emitted,':','LineWidth',1.5,'Color',[0.8500 0.3250 0.0980])
% hold on
% plot(time,Qtarget,':','LineWidth',1.5)
% plot(time,Qmac_needed_plot,'-','LineWidth',1.5)
% legend('Received cabin','Emitted','Target','MAC needed','Location','best')
% grid minor
...
plot(time,Qemitted,'-','LineWidth',1.5,'Color',[0.3010 0.7450 0.9330])
ylabel('Thermal Loads [W]')
xlabel('Time [s]')
yyaxis right
ax = gca
...
legend('Cabin Total','Ambient','Human','Equipment','Vents','Engine',...
    'Exhaust','Leakage','Overheat','Cabin Received','Cabin Emitted','MAC','Location','northeastoutside')
grid minor
% xlim([0 1800])
...
if(G7_22==1)
    plot(time,T_cell,'-k','LineWidth',1) %T amb 
else
    plot(time,T_amb-273.15,'-k','LineWidth',1) %T amb 
end
