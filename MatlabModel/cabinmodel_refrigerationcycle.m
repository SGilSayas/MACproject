...
clear py;
%%
% First version combining th cabin model and the refrigertion cycle
% Cabin model: cabinmodelfunction4 (cabinmodel_v22)
...
%
% Gil-Sayas, S., 2025
%% If message not finding CoolProp: Redo link python-matlab
% in matlab terminal: 
% pyenv('Clear');
...
% Delta T between the heat exchangers (evaporator and condenser) and
%   the cold and hot sources (cabin and external ambient air)
...
%% Values from the cabin model
T_amb = ones(1,Total_time+1)*26 + 273.15; %C % Row vecto
T_ini = T_amb(1); % alt: T_ini = TC_cabin_ini + 273.15;

% Evap heat from cabin model %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Qmac = ones(1,Total_time+1);
...
C_sidewindows=0;
C_doors=0;
C_cabin_front = density_air*cp_air/timestep*V_cabin*0.9;
C_cabin_back = density_air*cp_air/timestep*V_cabin*0.001;
C=zeros(14);
C(1,1)=C_amb;
C(2,2)=C_roof;
C(3,3)=C_ceiling;
...
U_front=ones(1,Total_time+1);
U_side=ones(1,Total_time+1);
U_back=ones(1,Total_time+1);
...
for t=2:Total_time+1 % better for for the mf
    ...
    if(size(T_amb)==[1 1])
        Tbc=[T_amb;-A_roof*G_roof_a(t);0;-Qbase(t);-A_ws*G_ws_a(t);-A_rw*G_rw_a(t);...
            -A_sidewindows*G_sidewindows_a(t);-A_doors*G_doors_a(t);0;0;0;0;...
            -Qhuman(t)-Qengine_av(t)-Qvent(t)+Q_MAC(t)-Qcv_emitted(t);-Qequipment(t)-Qexhaust_av(t)-Qleakage(t)];
    else 
    ...
    Qcabin_received(t) = Qhuman(t) + Qequipment(t) + Qengine_av(t) + Qexhaust_av(t) + Qcv_received(t) + Qvent(t) + Qleakage(t);
    Qcabin_req(t) = Qcabin_received(t) + Qcv_emitted(t);
    ...
    if (P2(t) <= compression_ratio_min*P1(t))
    ...
    %old COP:COP(t)=(h2(t)-h3(t))/(h2(t)-h1(t));
    ...
    temperature(13) = (Qcabin_req(t) - Q_MAC(t))/2*(timestep/(V_cabin/2*cp_air*density_air)) + prev_temp(13);
    temperature(14) = (Qcabin_req(t) - Q_MAC(t))/2*(timestep/(V_cabin/2*cp_air*density_air)) + prev_temp(14);
...
figure(7)
plot(time(3:Total_time),Qcabin_req(3:Total_time),'LineWidth',1.5)
hold on
...
ax.YColor = [1 0 0];
hold on
plot(time(3:Total_time),Qcabin_req(3:Total_time),'-r','LineWidth',1.5)
