...
for t=2:Total_time+1
    ...
    Qengine_W(t) = cp_air*CabinVolume/2*density_air*delta_temp_engine_mean(t)/timestep;
    Qtailpipe_W(t) = cp_air*CabinVolume/2*density_air*delta_temp_tailpipe_mean(t)/timestep;
        % %Checking heats:
        % Qhuman(t) =0;
        % Qequipment(t)=0;
        % Qengine_W(t)=0;
    ...
    if(Irr==0)
        % IF T amb is a constant value:
        if(size(T_amb)==[1 1]) 
            Tbc=[T_amb;0;0;-Qbase(t);0;0;0;0;0;0;0;0;-Qhuman(t)-Qengine_W(t);-Qequipment(t)-Qtailpipe_W(t)];

        % elseif t<=Total_time+1; % checking if T bc is causing the t=2 step
        %   Tbc=[T_amb(t);0;0;-Qbase(t);0;0;0;0;0;0;0;0;-Qhuman(t)-Qengine_W(t);-Qequipment(t)-Qtailpipe_W(t)];

        %   Tbc=Tbc*0;

        % IF T amb is a vector from input (size(T_amb)==[1 1801]):
        else 
            Tbc=[T_amb(t);0;0;-Qbase(t);0;0;0;0;0;0;0;0;-Qhuman(t)-Qengine_W(t);-Qequipment(t)-Qtailpipe_W(t)];
        end

    % IF there is irradiance and T amb is a vector:
    else 
        ...
        Tbc=[T_amb(t);-A_roof*G_roof_a(t);0;-Qbase(t);-A_ws*G_ws_a(t);-A_rw*G_rw_a(t);...
            -A_sidewindows*G_lsw_a(t);-A_doors*G_rsw_a(t);0;0;0;0;-Qhuman(t)-Qengine_W(t);-Qequipment(t)-Qtailpipe_W(t)]; 
    end
    ...
    Eeng_kWh(t) = trapz(Qengine_W(:,1:t))*t / (3600*1000);
    Etail_kWh(t)= trapz(Qtailpipe_W(:,1:t))*t / (3600*1000);
...
% Ttarget = 22 + 273.15; %K
% T0cabin = Tcabin(0); %K
% ttarget = ; %time to reach the Ttarget from T0 cabin
% tcons= ttarget/log(abs(T0cabin-Ttarget)); % pull-down constant: overall pull-down time
% 
% Q_MAC(t) =-(Qhuman + Qequipment + Qengine_W + Qtailpipe_W + Qcv_cabin)...
%     -(CabinVolume*density_air*cp_air + mass_seats*cp_seats)*(Tcabin(t)-Ttarget)/tcons; % + 144240 ?
