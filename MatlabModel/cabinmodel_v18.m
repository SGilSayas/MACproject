...
%% Test conditions
Irr = 0; %-2.55; % IN NEGATIVE as it enters the cabin [W/(m2)] 
...
if(WT==1)
    ...
    wt_data=table2array(wt_heating);
    wt_heating_rs=ones(length(wt_data)*2,8);
    j=1;
    while j<length(wt_data)
        for k=1:length(wt_data)*2-1 %1800
            if rem(k,2) ~= 0 % for odd (impar) k
                wt_heating_rs(k,:) = wt_data(j,:);
            else % for even k
                wt_heating_rs(k,:) = wt_data(j,:)+((wt_data(j+1,:)-wt_data(j,:))./2);
                j=j+1;
            end
    ...
    TC_front = wt_heating_rs(1:end-6,4)+0.2;
    TC_driver = wt_heating_rs(1:end-6,5);
    TC_pass = wt_heating_rs(1:end-6,6);
    TC_back = wt_heating_rs(1:end-6,7)+0.2;
...
for t=2:Total_time
    ...
    elseif vehicle_kmh(t)>56.5 && vehicle_kmh(t)<=76.6 % MEDIUM, 20 m/s
        h_front(t) = 1000;
        U_front(t) = 100;
        U_back(t) = 10; % it doesnt matter the num,
