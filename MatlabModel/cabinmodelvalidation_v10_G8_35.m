...
% engineheatinput='Engine_thermal_loads_W_35G7_v9'; %file non existing anymore
...
TC_amb=transpose(TC_cell)+273.15; % T_amb in K. Both TC_cell or T_cell_OBD
T_ini=TC_amb(1);
...
UA_front= (k_ws*A_ws)/e_ws + 1*(k_sidewindows*A_sidewindows)/(e_sidewindows) ;
...
for t=2:Total_time
    ...
    % U_engine = h_cabin; % h_ext;
    % U_exhaust = h_cabin;

    e_base = 0.1; %m
    U_exhaust = 1/( 1/1 + 1/1 + e_base/k_steel );
    % 1/U = 1/h_engine + 1/h_cabina + e_chapa/k_metal; %buscar literatura.
    e_enginesheet = 0.01; %engine sheet thickness, m
    h_engine = 5; % h_engine = f( velocidad aire que pasa por el motor )
    U_engine = 1/( 1/1 + 1/1 + e_enginesheet/k_steel );
...
% Qengine_observed_G8_35 = Qengine_observed;
% Qexhaust_observed_G8_35 = Qexhaust_observed;
% save Engine_thermal_loads_W_G8_35_v10 Qengine_observed_G8_35 Qexhaust_observed_G8_35
...
plot(time,TC_amb-273.15,'-k','LineWidth',1) %T amb 
