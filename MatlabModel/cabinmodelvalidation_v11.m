...
% Want to reduce the amplitud of T amb?
...
if(G7_22==1)
    reduced_amplitude = 1; % 1 for yes, 0 for no
    reduction = 5; % Highly recommended for G7_22
...
if(G7_22==1)
    bias_front = - abs(TC_front(2)-TC_amb(2));
    bias_back = - abs(TC_back(2)-TC_amb(2));
elseif(G7_35==1)
    bias_front = abs(TC_front(2)-TC_amb(2)) + 0.5;
    bias_back = abs(TC_back(2)-TC_amb(2)) + 0;
elseif(G8_22==1)
    bias_front = -(25-21.8);
    bias_back = -(25.8-21.6);
elseif(G8_35==1)
...
TC_front= TC_front + bias_front;
TC_back= TC_back +bias_back;
...
for t=2:Total_time
   ...
   % Exchange from the front and back:
    ...
    % Thermal Loads
    % Qambient(t) = abs(T_amb(t)-temperature(13))*UA_front + abs(T_amb(t)-temperature(14))*UA_back ;
    Qambient(t) = Qcv_received(t);
