...
%%
% Octave version:
% do it once: pkg install -forge io
%
% Working on V4:
...
fluid = {'R1234yf'}; %{'Water'}; % %{'R134a'};
  % R1234yf_op_pres = odsread('R1234yf_operating_pressures.xlsx'); %trials: xlsread,readmatrix
  % headers = {'Column1', 'Column2', 'Column3'};  % Define your own column names
  % R1234yf_op_pres = csvread('filename.csv', 1, 0);  % Skip the first row (assumed to be headers)
R1234yf_op_pres = csv2cell('R1234yf_operating_pressures_csv.csv');
headers = R1234yf_op_pres(1, :);
R1234yf_op_pres = cell2mat(R1234yf_op_pres(2:end, :));
R1234yf_op_pres(:, 1) = 273.15 + R1234yf_op_pres(:, 1);
...
%%
    ...
    idx = find(R1234yf_op_pres(:, 1) == T_amb(t));

%T_ambient_C = R1234yf_op_pres(idx, 1);
%P_LowSide_min_kPa	= R1234yf_op_pres(idx, 2);
%P_LowSide_max_kPa	= R1234yf_op_pres(idx, 3);
%P_HighSide_min_kPa	= R1234yf_op_pres(idx, 4);
%P_HighSide_max_kPa = R1234yf_op_pres(idx, 5);

    if ~isempty(idx) % If T_amb is found in the table
        P_LowSide_max_Pa = 1000*R1234yf_op_pres(idx, 3);
        P_HighSide_max_Pa = 1000*R1234yf_op_pres(idx, 5);
    else % If T_amb is not found, interpolate
        T_ambient = R1234yf_op_pres(:, 1);
        P_LowSide_max_Pa = 1000*R1234yf_op_pres(:, 3);
        P_HighSide_max_Pa = 1000*R1234yf_op_pres(:, 5);

        if length(T_ambient) >= 2
          P_LowSide_max_Pa = interp1(T_ambient, P_LowSide_max_Pa, T_amb, 'linear', 'extrap');
          P_HighSide_max_Pa = interp1(T_ambient, P_HighSide_max_Pa, T_amb, 'linear', 'extrap');
        else
