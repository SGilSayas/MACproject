% Vehicle's Cabin Model, 1st:April 2021
clc
clear all 
close all
...
t = 1;
timestep =  1; %s
TC_input='TC_inputs.mat';
struct=load(TC_input);
table=struct2table(struct);
TC=table2array(table);
TC_vent=transpose(TC.Thermocouple11);
TC_front=transpose(TC.Thermocouple12);
TC_driver=transpose(TC.Thermocouple13);
TC_pass=transpose(TC.Thermocouple14);
TC_back=transpose(TC.Thermocouple15);
TC_backpass=transpose(TC.Thermocouple16);
T_ambient=transpose(TC.TC_Cell_2);
...
% Initial conditions
T_ini(:,t) = 273.15 + T_ambient(:,t); % [K], initial value for simulation
T_sky = T_ini - 6; % [K]
...
% (Emisia 56 W/m2)
random=0.15;
Human_W=56*1.8*random; %
Qhuman = ones(1,Total_time+1) * N_Humans * Human_W; % [W]
...
plot(time,TC_back)
hold off
legend('T front sim','T back sim','T roof sim','T ambient sim','TC front','TC back','Location','best')
grid on 
grid minor
xlabel('Time [s]') 
