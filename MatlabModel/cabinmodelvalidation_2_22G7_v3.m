...
% simulated Tback shows too many peaks
clc
clear all 
close all
...
A_tot = (A_ws + A_rw + A_sidewindows/2 + A_doors/2 + A_roof/2);
e_av = (e_ws + e_rw + e_sidewindows/2 + e_doors/2 + e_ceiling/2)/5 ;
...
e_av_back = (e_rw + e_sidewindows/2 + e_doors/2 + e_ceiling/2)/4 ;
...
for t=2:Total_time+1
    ...
    if (temperature(13) > T_amb(t) + i ) % && temperature(14)<temperature(13))
        %R_front= 1/h_cabin + e_av/(k_av) + 1/h_ext; U_front= 1/R_front;
    ...
    if (temperature(14) > T_amb(t) + 3 ) % && temperature(14)>temperature(13))
        ...
        temperature(14) = (UA_back*temperature(1) + density_air*V_cell*delta_temperature(14)/timestep) / (UA_back + density_air*V_cell/(timestep));         %temperature(14) = (h_cabin*cabin_height*cabin_width*temperature(13) + density_air*CabinVolume*delta_temperature(14)/timestep) / (h_cabin*cabin_height*cabin_width + density_air*CabinVolume/(timestep));
