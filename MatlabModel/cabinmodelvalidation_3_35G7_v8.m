...
% v8: RH and P inputs, enthalpies, ventilation mass & heats - line 200
clc
clear all 
close all
...
% Cabin overall heat transfer coefficients, UA (W/K):
...
%% Load inputs for validation: Golf 7 22C: LAB_20220511_06_MAC_on
...
time_Modaldata = table2array(modalxcu_array(:,11));
n=1;
...
TC_front= TC_front + abs(T_amb(2)-273.15-TC_front(2));
TC_front = movmean(TC_front,5);
TC_back= TC_back + abs(T_amb(2)-273.15-TC_back(2));
...
delta_temp_engine_mean(t) = 1;
delta_temp_tailpipe_mean(t) = 1;
T_engine(t)=T_ini;
T_exhaust(t)=T_ini;
check=0; 
check_f=0; 
...
% Qcv_received = ones(1,Total_time);
% Qvent = ones(1,Total_time);
% Qengine = ones(1,Total_time);
% Qexhaust = ones(1,Total_time);
% Qengine_observed = ones(1,Total_time);
% Qexhaust_observed = ones(1,Total_time);
% Qengine_calc = ones(1,Total_time);
% Qexhaust_calc = ones(1,Total_time);
...
vent_volumerate=0.001; % m3/s, Faya 2013: 0.02; 0.001
...
for t=2:Total_time
    ...
    % Heats:
        ...
        % Qexhaust(t)=0;
        % Qvent(t)=0;
    Qvent(t)=-vent_massflowrate*(e_amb(t)-e_cabin(t)); % W, J/s
    ...
    Qengine_calc(t)= A_engine*U_engine*(T_engine(t)-temperature(13)) - 11; %- 55.2;
    Qexhaust_calc(t)= A_exhaust*U_exhaust*(T_exhaust(t)-temperature(14)) + 17.77;


    %% OP 2: works very well, but does it make sense? v8 aim is to delete it.
    if ( TC_front(t)+273.15>temperature(13) )
        delta_temp_engine_mean(t) = (TC_front(t)+273.15-temperature(13)); % K
    else
        delta_temp_engine_mean(t) = abs(TC_front(t)+273.15-temperature(13));
    end
    Qengine_observed(t) = cp_air*CabinVolume/2*density_air*delta_temp_engine_mean(t)/timestep;

    delta_temp_tailpipe_mean(t) = (TC_back(t)+273.15-temperature(14)); % K
    Qexhaust_observed(t) = cp_air*CabinVolume/2*density_air*delta_temp_tailpipe_mean(t)/timestep;
    
    %% OP 3: nope
    % Qengine_observed(t) = UA_front * delta_temp_engine_mean(t);
    % Qexhaust_observed(t) = UA_back * delta_temp_tailpipe_mean(t);
    %%
        % Qengine(t)=0;
        % Qexhaust(t)=0;
    ...
    %%
    % Engine energy:
    % Eeng_kWh(t) = trapz(Qengine(:,1:t))*t / (3600*1000);
    % Etail_kWh(t)= trapz(Qexhaust(:,1:t))*t / (3600*1000);
    %cumW=trapz(Qengine_W(:,1:t);

    % Boundary conditions vector:
    if(Irr==0)
        if(size(T_amb)==[1 1]) % IF T amb is a constant value:
            Tbc=[T_amb;0;0;-Qbase(t);0;0;0;0;0;0;0;0;-Qhuman(t)-Qengine(t);-Qequipment(t)-Qexhaust(t)];
        else % IF T amb is a vector from input (size(T_amb)==[1 1801]):
            Tbc=[T_amb(t);0;0;-Qbase(t);0;0;0;0;0;0;0;0;-Qhuman(t)-Qengine(t);-Qequipment(t)-Qexhaust(t)];
        end
        ...
        Tbc=[T_amb(t);-A_roof*G_roof_a(t);0;-Qbase(t);-A_ws*G_ws_a(t);-A_rw*G_rw_a(t);...
            -A_sidewindows*G_lsw_a(t);-A_doors*G_rsw_a(t);0;0;0;0;-Qhuman(t)-Qengine(t);-Qequipment(t)-Qexhaust(t)]; 
    end
    ...
    if ( temperature(13)>T_amb(t) ) 
        temperature(13)= (UA_front*T_amb(t)*0.998 + density_air*V_cell*delta_temperature(13)/timestep)...
            / (UA_front + density_air*V_cell/(timestep));% if T_amb(t)*0.998 all ok
    ...
    % Thermal load in the cabin:
    Qcabin_received(t)=Qhuman(t) + Qequipment(t) + Qengine(t) + Qexhaust(t) + Qcv_received(t) + Qvent(t);% + Qcv_emitted(t);

    % Thermal load needed from MAC:
    % Qmac_needed(t)=(CabinVolume*density_air*cp_air + mass_seats*cp_seats+144240)*(((temperature(13)+temperature(14))/2)-Ttarget)./tcons(t);
    % 
    % % Time to reach the T target, ttarget:
    % Qmac_needed(t) = Qcabin_received(t);
    % ttarget(t) = log(abs(T0cabin-Ttarget)) * (CabinVolume*density_air*cp_air + mass_seats*cp_seats+144240)*(Tcabin(t)-Ttarget)/Qmac_needed(t);
    % tcons(t)= ttarget(t)/log(abs(T0cabin-Ttarget));

end

% trQengine_W = transpose(Qengine_W);
% trQtailpipe_W = transpose(Qtailpipe_W);
% save Qengine_Qtailpipe_W_35G7_v6 trQengine_W trQtailpipe_W
...
legend('Qengine observed','Qexhaust observed','Q vent','Qengine calc','Qexhaust calc')
%%
% figure(6)
% plot(time,Qcabin_received) 
% xlabel('Time [s]') 
% hold on
...
%%
% figure(5)
% plot(time,Qcabin_received-abs(Qcv_emitted)) 
% xlabel('Time [s]') 
% hold on
...
% figure(4)
% plot(time,Qcabin_received) 
% ylabel('Heat received from the ambiebt to the cabin [W]')
% xlabel('Time [s]') 
% hold on
% yyaxis right
% plot(time,Qcv_emitted)
% ylabel('Heat emitted from the cabin to the ambient [W]')
% legend('Received','Emitted')
%% 
% figure(3)
% xlabel('Time [s]') 
% plot(time,Qmac_needed)
