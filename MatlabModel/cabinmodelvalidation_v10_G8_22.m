...
n=height(lab_table)/duration_WLTC; %n=10.03
...
TC_vent=table2array(lab_table1(:,1)); % C
TC_front=table2array(lab_table1(:,2)); % C
TC_driver=table2array(lab_table1(:,3)); % C
TC_pass=table2array(lab_table1(:,4)); % C
TC_back=table2array(lab_table1(:,5)); % C
TC_backpass=table2array(lab_table1(:,6)); % C
...
TC_backpass=TC_backpass(6 : n : end);
TC_vent=TC_vent(:,:);
TC_front=TC_front(:,:);
TC_driver=TC_driver(:,:);
TC_pass=TC_pass(:,:);
...
for t=2:Total_time
    ...
    Qengine_observed_G8_22(t) = cp_air*CabinVolume/2*density_air*(delta_temp_engine_mean(t))/timestep;
    Qexhaust_observed_G8_22(t) = cp_air*CabinVolume/2*density_air*(delta_temp_exhaust_mean(t))/timestep;
   ...
   % Exchange from the front and back:
    ...
    Qmac_needed(t) = Qcabin_received(t) + Qcv_emitted(t);
...
% save Engine_thermal_loads_W_G8_22_v10 Qengine_observed_G8_22 Qexhaust_observed_G8_22
