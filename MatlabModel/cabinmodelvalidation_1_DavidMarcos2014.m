...
% April 2021
% May 2021, upgrade to lumped model (14 nodes). Modified by Susana Gil-Saya and Amin Dreif 
clc
clear all 
close all
%% INPUTS
...
sigma=0.0000000567; %5.67037321*10^-8 W/(m^2*K^4) %Stefan-Boltzmann constant(sigma)
...
cabin_r_l = 1.8;
cabin_b_l = 1.45;
CabinVolume = cabin_w*(cabin_r_l + cabin_b_l)*cabin_h/2; % = 1.45*1.3*1;
...
Irr = interp1(Irradiancia(:,1),Irradiancia(:,2),time); % 1x13201
T_ext = 273.15 + interp1(Temp_Ext(:,1),Temp_Ext(:,2),time);
T_amb = T_ext;

% Human heat
N_Humans=0; % In David Marcos at al, 2014, there's no human
human_heat=80;
Qhuman(t)=N_Humans * human_heat; %65*N_humans; %sgs W, UNE-EN ISO 8996, Alberto Viti Corsi (IDAE)
% for j=1:Total_time+1
% Qhuman(j)=Qhum; 
% end
...
G_ws_a=alfa_ws*G_ws_inc; %añadir como bc
...
% Initial Boundary conditions vector
...
K1=h_ext*A_roof; %h_roof x q exite?
...
plot(T_Air_Mod_Ref(:,1),T_Air_Mod_Ref(:,2),':r','LineWidth',1.5)
plot(T_Air_Exp(:,1),T_Air_Exp(:,2),'-r','LineWidth',1.5)
plot(time,T_amb-273.15,'-k','LineWidth',1.5)
legend('T cabin front sim','T cabin back sim','T cabin sim (D.M.2014)','TC cabin (D.M.2014)','TC amb','Location','best')
grid on
grid minor
xlabel('Time [s]') 
ylabel('Temperature [ºC]')
axis([0 13130 20 55]) %xx yy
