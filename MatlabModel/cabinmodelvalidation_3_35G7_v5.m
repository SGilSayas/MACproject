...
TC_front= TC_front + abs(T_amb(1)-273.15-TC_front(1));
TC_back= TC_back + abs(T_amb(1)-273.15-TC_back(1));
...
%     % Exchange from the front and back
    if (temperature(13)>T_amb(t) )% && temperature(14)<temperature(13))
        R_front= 1/h_cabin + e_ws/(k_ws) + e_sidewindows/k_sidewindows + 1/h_ext;
        U_front= 1/R_front;
        temperature(13) = (U_front*2*A_ws*temperature(1) + density_air*V_cell*delta_temperature(13)/timestep) / (U_front*2*A_ws + density_air*V_cell/(timestep));
    ...
    if (temperature(14)>T_amb(t) )%&& temperature(14)>temperature(13))
        R_back= (1/h_cabin + e_rw/(k_rw) + 1/h_ext);
        U_back= 1 * 1/R_back ;
        ...
        temperature(14) = (U_back*2*A_rw*temperature(1) + density_air*V_cell*delta_temperature(14)/timestep) / (U_back*2*A_rw + density_air*V_cell/(timestep));         %temperature(14) = (h_cabin*cabin_height*cabin_width*temperature(13) + density_air*CabinVolume*delta_temperature(14)/timestep) / (h_cabin*cabin_height*cabin_width + density_air*CabinVolume/(timestep));
    ...
    Q_cv_left(:,t)=-h_lsw*A_sidewindows*(Tw_int_lsw(:,t)-Tcabin_back(:,t));
    Q_cv_right(:,t)=-h_rsw*A_doors*(Tw_int_rsw(:,t)-Tcabin_back(:,t));
