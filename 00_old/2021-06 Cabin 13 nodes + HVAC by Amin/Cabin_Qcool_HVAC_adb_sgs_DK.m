...
% Vel = interp1(Veh_speed(:,1),Veh_speed(:,2),time); Vel = time *0 + 5;
...
for t=2:Total_time+1
    ...
    Qcool_req(t)=rho_air*V_cabin*cp_air*(temperature(13)-Ttarget)/timestep; %Tcabin=324.7876
...
Qcool_actual(t)=rho_air*V_cabin*cp_air*(Tcabin(:,t)-Ttarget)/timestep;
...
figure(2)
% yyaxis left
plot(time,Tcabin-273.15,'o')
hold on
plot(time,Tamb-273.15)
grid on
grid minor
...
legend('T cabin','T ambient (b.c.)','T target','Qcool actual','Location','NorthWest')
