...
Tamb_C = +50; %T ambient in celsius
for j=1:Total_time
Tamb(j) = Tamb_C + 273.15;
%Irr(j)=0;
end
T_ini=Tamb_C + 273.15;%Tamb(1);

% Human heat
N_human = 1;
Qhuman(t)=N_human * 120; 
...
for t=2:Total_time+1
    ...
    %Q(t)=rho_air*V_cabin*cp_air*(temperature(13)-Ttarget)/timestep; %Tcabin=324.7876
    Q_req(t)=rho_air*V_cabin*cp_air*(temperature(13)-Ttarget)/timestep;
    Tbc = [Tamb(t);-A_roof*G_roof_a(t);0;-Qbase(t);-A_ws*G_ws_a(t);-A_rw*G_rw_a(t);-A_lsw*G_lsw_a(t);-A_rsw*G_rsw_a(t);0;0;0;0;-Qhuman(t)+Q_req(t)]; 
    
    temperature=(inv(K - C))*(Tbc - C*delta_temperature);
    delta_temperature=temperature;
    
    Tamb(:,t)=temperature(1);
...
Q(t)=rho_air*V_cabin*cp_air*(Tcabin(:,t)-Ttarget)/timestep;
Q_cum=Q(t-1)+Q(t);
end
%fila error=Qcool_actual-Qcool_req; % %if loop: % Qcool_req=Qcool_actual;% E(i,:)=error;% TCABIN(i,:)=Tcabin;% i=i+1; %contador de cada run
T_cabin_final=Tcabin(:,Total_time); %RUN#1: 52ºC; %RUN#2: 324.8446K=51.6946ºC ; %RUN#3: 324.8446

%amin: Tcabin(t) = (Q_windows(t)+Q_ceiling(t)+Qbase(t)+Qhuman(t) - Q_HVAC(t) )*(timestep/(V_cabin*cp_air*rho_air)) + Tcabin(t-1);

% ymin=min(Tcabin)-275.15-1;
% ymax=max(Tcabin)-273.15+1;
figure(2)
% yyaxis left
plot(time(2:end),Tcabin(2:end)-273.15,'.')
% hold on
% plot(time,Tamb)
grid on
grid minor
...
% xlim([-100 13300])
% yyaxis right
% plot(time(2:end),Qcool_actual(2:end),'k')
% ylabel('Cooling heat [W]')
%legend('T cabin','T ambient (b.c.)','Q','Location','NorthWest')
legend('T cabin','Q','Location','NorthWest')
