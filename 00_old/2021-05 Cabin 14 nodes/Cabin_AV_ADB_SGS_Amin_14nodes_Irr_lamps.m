...
A_surface=A_roof+A_lsw+A_rsw+A_rw+A_ws;
...
T_ext = ones(1,13201)*(28+273.15);
T_amb = T_ext;
Irr_lamp=850; % W/(m2*K)
Irr = ones(1,13201)*Irr_lamp;

% Human heat
N_Humans=1;
Qhuman(t)=N_Humans * 120; %65*N_humans; %sgs W, UNE-EN ISO 8996, Alberto Viti Corsi (IDAE)
...
figure(3)
plot(time,Tcabin_front-273.15,'b',time,Tcabin_back-273.15,'r')
hold on
plot(time,Troof-273.15,time,T_amb-273.15)
legend('T cabin front','T cabin back','T roof','T ambient (b.c.)','Location','best')
grid on
grid minor
xlabel('Time [s]') 
ylabel('Temperature [ºC]')
